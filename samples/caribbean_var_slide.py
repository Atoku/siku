...
## custom testing polygons for caribbeans # lon, lat convention
    ...
    for i in range( len( NN ) ):
        cds, links = NG.generate_plus( \
            285.0-sx*i, 9.0+i*5.0, 295.0-sx*i, 13.0+i*5.0, \
            nx[i], ny[i], 0., 0. )
...
##
    ...
    for i in range( len( NN ) ):
        cds, links = NG.generate_plus( \
            285.0-sx*i-10.0/nx[i], 9.0+i*5.0, 285.0-sx*i, 13.0+i*5.0, 1, ny[i], \
            0., 0. )
        coords = coords + cds
        ...
        cds, links = NG.generate_plus( \
            295.0-sx*i, 9.0+i*5.0, 295.0-sx*i+10.0/nx[i], 13.0+i*5.0, 1, ny[i], \
            0., 0. )
        coords = coords + cds
        t = [ n_filled+j for j in range(ny[i]) ]
...
##                                    for i in right_inds ]
    ...
    for i in right_inds:
        siku.elements[i].velo = (0.0, 2.0, 0.0)

...
Sikupy::~Sikupy()
{
  ...
  if ( flag & FLAG_INITIALIZED ) finalize();
}
...
int Sikupy::read_info( Info& info )
{
  int success = 1;
...
//---------------------------------------------------------------------
int Sikupy::read_planet( Planet& planet )
{
  int success = 1;
  int status;
  ...
  if ( !PyObject_IsInstance( pSiku_planet, pSiku ) )
    fatal( 1, "Wrong siku.planet option,"
           " must be an instance of siku.planet.Planet class" );
...
//---------------------------------------------------------------------
int Sikupy::read_modeltime( ModelTime& time )
{
  int success = 1;
  ...
  if ( !status ) fatal( 1, "Failed to read siku.time.start" );
  ...
  assert( pobj );
  status = read_time( pobj, tmp_t );
  if ( !status ) fatal( 1, "Failed to read siku.time.finish" );
  Py_DECREF( pobj );            // done with it
  ...
  if ( !status )
    fatal( 1, "Failed to read siku.time.dt"
           " (must be datetime.delta instance)" );
  Py_DECREF( pobj );           // done with it
  ...
  status = read_dt( pobj, tmp_dt );
  if ( ! status )
    fatal( 1, "Failed to read siku.time.dts"
           " (must be datetime.delta instance)" );
  Py_DECREF( pobj );           // done with it
...
int Sikupy::read_materials( vector<Material>& ms )
{
  int success = 1;
...
int Sikupy::read_elements( vector<Element>& es )
{
  int success = 1;
...
int Sikupy::read_diagnostics( Diagnostics& diag )
{
  int success;
...
int Sikupy::read_diagnostics_meshes( Diagnostics& diag )
{
...
int Sikupy::read_diagnostics_winds( Diagnostics& diag )
{
  int success = 1;
  ...
  else diag.flags_set( Diagnostics::WIND_ON );
  ...
  for ( Py_ssize_t i = 0; i < M; ++i )
    {
      ...
      if ( !PyTuple_Check( pitem ) ||
           PyTuple_Size( pitem ) != 4 )
        fatal( 2,
               "diagnostics.wind element %u is not a tuple size 4",
               (unsigned int)i );
      ...
      if ( ! PyCallable_Check( psubitem ) )
        fatal( 3,
               "diagnostics.wind element %u element 0 is not a function",
               (unsigned int)i );
      ...
      if ( !bstatus )
        fatal( 4, "could not read mesh index"
               " from diagnostics.wind at element %u", (unsigned int)i );
      ...
      if ( !bstatus )
        fatal( 4, "could not read initial time"
               " from diagnostics.wind at element %u", (unsigned int)i );

      psubitem = PyTuple_GetItem( pitem, 3 );
      bstatus = read_dt( psubitem, sdt );
      if ( !bstatus )
        fatal( 4, "could not read time frequency"
               " from diagnostics.wind at element %u", (unsigned int)i );
...
int Sikupy::fcall_monitor( const Globals& siku,
                           const size_t i,
                           const char* fname )

{
  ...
  if ( !pReturnValue ) PyErr_Print();
  assert( pReturnValue );

  if ( pReturnValue ) Py_DECREF( pReturnValue );
  Py_DECREF( pCurTime );
  Py_DECREF( pQTuple );
  Py_DECREF( pPiList );
  Py_DECREF( pFunc );
...
int Sikupy::fcall_diagnostics_vec3d( const Globals& siku,
                                     const size_t i,
                                     const vector<vec3d>& data )
{
  ...
  if ( pReturnValue ) Py_DECREF( pReturnValue );
  if ( pCurTime )     Py_DECREF( pCurTime );
  if ( pDataList )    Py_DECREF( pDataList );
...
bool Sikupy::read_quat( PyObject* pquat, quat& q )
{
  // check if it is a list
  if ( !PyList_Check( pquat ) ) return false;
  Py_ssize_t K = PyList_Size( pquat );

  if ( K != 4 ) return false; // must be 4 elements of q

  for ( Py_ssize_t k = 0; k < K; ++k )
    {
      PyObject* pitem;
      pitem = PyList_GetItem( pquat, k ); // borrowed
      if ( !PyFloat_Check( pitem ) ) return false;
...
bool Sikupy::read_long( PyObject* plong, long& x )
{
...
bool Sikupy::read_string( PyObject* pstring, string& str )
{
  if ( !PyUnicode_Check( pstring ) ) return false;
...
bool Sikupy::read_double_vector( PyObject* plist, vector<double>& xs )
{
 ...
 */
bool Sikupy::read_vec3d_vector( PyObject* plist, vector<vec3d>& vs )
{
  assert ( PyList_Check( plist ) );
  Py_ssize_t N = PyList_Size( plist );
       ...
       for ( Py_ssize_t k = 0; k < K; ++k )
         {
           ...
           if ( !PyFloat_Check( psubitem ) ) return false;
...
bool Sikupy::read_dt( PyObject* pobj,
                      boost::posix_time::time_duration& dt )
{
  if ( ! PyDelta_Check( pobj ) ) return false;
  ...
  assert( pitem );
  if ( ! PyLong_Check(pitem) ) return false;
...
bool Sikupy::read_time( PyObject* pobj,
                        boost::posix_time::ptime& t )
{
  int year, month, day, hour, minute, second, ms;

  if ( ! PyDateTime_Check( pobj ) ) return false;
  ...
  t = ptime( start_date, hours(hour) +
             minutes(minute) + seconds(second) + millisec(ms) );
